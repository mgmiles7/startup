...
export function Chat(props) {
    ...
    React.useEffect(() => {
        ...
        const id = setInterval(() => {
            ...
            const time = `${date} ${hours}:${minutes}`
            const id = now.getMilliseconds();
            const mess = new Message("slightly annoying filler message", props.user.with, time, id)
            props.setUser(prev => ({
                ...prev,
                messages: [...prev.messages, mess]
            }))
    ...
    <div className='main chat'>
        <h3>{props.user.with}</h3>       
            <ul className = "messages">
                {props.user.messages.map((item) =>
                <li key={item.id} className= {(item.sender === props.user.username) 
